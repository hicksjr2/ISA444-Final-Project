# Business Forecasting Using Statistical, ML, Deep Learning and Transformer Models

In this class project, we utilize the [Walmart Dataset from Kaggle]() to perform ..... Given that the dataset contained XYZ different time-series, we focused on ABC time-series. We picked these timeseries **as they** ... 

## Forecasting Procedure
Our approach consisted of the following steps:  
  1. ABC
  2. EFG
  3. a

## Results and Observations
ABCD
![]()

## Our Python Notebooks
The following notebooks contain our code, results and insights:  
  - [Downsamplin process](https://github.com/hicksjr2/ISA444-Final-Project/blob/main/ISA444_downsample_preprocess.ipynb)
  - 

